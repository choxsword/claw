...
#define TIME_OUT                   15         // Este parametro depende de la velocidad de transmision
...
#define LOCK                        1
#define ID							1
#define WAIT_RDT					100//跟 TX_DELAY_TIME有关
...
class DynamixelClass:public IDynamixel {
private:
...
public:
	...
	int moveRW( int Position);
	int moveSpeedRW( int Position, int Speed);
	...
	int readTemperature();
	int readVoltage();
	int readPosition();//hex
	int readSpeed();
	int readLoad();
	
	int torqueStatus( bool Status);
	int ledStatus( bool Status);
	bool is_cw=false;//顺时针旋转，或者是手爪开启
};
