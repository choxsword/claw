...
float Fuzzy_controller::get_e_pre() {
	return epre;
}

Fuzzy_controller::Fuzzy_controller(float _e_max,float _de_max,float _u_max):
target(0),actual(0),emax(_e_max),demax(_de_max),umax(_u_max)
{
   e=target-actual;
   epre=0;
   de=e-epre;
...
void Fuzzy_controller::set_para(float e_max, float de_max, float u_max){
	emax=e_max;
	demax=de_max;
	umax=u_max;
	Ke=3.0/emax;
...
float Fuzzy_controller::realize_test(float t, float a,float* para)
{
	//epre = 0;
	para[0] = epre;
	float u_e[N], u_de[N], u_u[N];
